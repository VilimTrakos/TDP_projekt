...
setup_replication_users() {
    ...
    RESPONSE=$(curl -s -X POST http://admin:password@$TARGET_HOST:5984/_replicate \
        -H "Content-Type: application/json" \
        -d "{
            \"source\": \"http://admin:password@$SOURCE:5984/_users\",
            \"target\": \"http://admin:password@$TARGET:5984/_users\",
            \"continuous\": true
        }")
    echo "Replication response: $RESPONSE"
}
...
setup_replication_users "couch_c" "couch_b" "couch_b"
setup_replication_users "couch_b" "couch_a" "couch_a"
...
echo "\nCreating test document (medical_records) on couch_c..."
curl -s -X PUT \
